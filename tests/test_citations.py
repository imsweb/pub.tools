...
from pub.tools.citations import book_citation
from pub.tools.citations import chapter_citation
from pub.tools.citations import conference_citation
from pub.tools.citations import journal_citation
from pub.tools.citations import monograph_citation
from pub.tools.citations import period
from pub.tools.citations import publication_citation
from pub.tools.citations import punctuate
from pub.tools.citations import report_citation
from pub.tools.schema import Abstract
from pub.tools.schema import BookRecord
from pub.tools.schema import ChapterRecord
from pub.tools.schema import ConferenceRecord
from pub.tools.schema import JournalRecord
from pub.tools.schema import MonographRecord
from pub.tools.schema import Person
from pub.tools.schema import ReportRecord
...
class TestCitations:
    ...
    def test_citation_basics(self):
        start = ""
        expected = ""
        assert punctuate(start, "") == expected
        start = "foo"
        expected = "foo."
        assert punctuate(start, ".") == expected
        start = "foo+"
    ...
    def test_journal_citation(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. <i>Sample Journal</i> Jan 2007;4(5):345-7.</span>'
        )
        assert citation == journal_citation(html=True, publication=record)

        record.issue = ""
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. <i>Sample Journal</i> Jan 2007;4:345-7.</span>'
        )
        assert citation == journal_citation(html=True, publication=record)
        ...
        record.volume = ""
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. <i>Sample Journal</i> Jan 2007;(5):345-7.</span>'
        )
        assert citation == journal_citation(html=True, publication=record)
    ...
    def test_journal_link_citation(self):
        record = {
            "title": "My title",
            "authors": [
                {"lname": "Wohnlich", "iname": "E", "fname": "Eric"},
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. <a class="citation-pubmed-link" '
            'href="https://pubmed.ncbi.nlm.nih.gov/12345678/">My title</a>. '
            "<i>Sample Journal</i> Jan 2007;4(5):345-7.</span>"
        )
        assert citation == journal_citation(html=True, link=True, **record)
    ...
    def test_journal_abstract_citation(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. <i>Sample '
        ...
        assert citation == journal_citation(html=True, use_abstract=True, publication=record)
    ...
    def test_book_citation(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. First Edition. '
            "Van Halen E, editor. New York: "
            "Doubleday; 2007 Dec. p. 243. (My series)</span>"
        )
        assert citation == book_citation(html=True, publication=record)

        record.pubdate = ""
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. First Edition. Van Halen E, '
            "editor. New York: "
        ...
        record.authors = []
        citation = (
            '<span class="citation">Van Halen E, editor. My title. First Edition. New York. p. 243. (My series)</span>'
        )
        assert citation == book_citation(html=True, publication=record)
    ...
    def test_chapter_citation(self):
        record = ChapterRecord(
            ...
            editors=[Person(last_name="Van Halen", first_name="", initial="E")],
            edition="First Edition",
            pubdate="2007 Dec",
            publisher="Doubleday",
            pubplace="New York",
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My title. In: Van Halen E, editor. '
            "My Book. First Edition. "
            "New York: Doubleday; 2007 Dec. p. 243. (My series)</span>"
        )
        assert citation == chapter_citation(html=True, publication=record)
        ...
        record.authors = []
        citation = (
            '<span class="citation">Van Halen E, editor. My title. In: My Book. First Edition. '
            "New York. p. 243. "
            "(My series)</span>"
        )
        assert citation == chapter_citation(html=True, publication=record)
    ...
    def test_conference_citation(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Battle J. My title. Sagan C, Thorne K, editors. '
            "<i>Proceedings of Conference "
            "name</i>; 2007 Dec; New York. Boston: Doubleday; 2008 Jan. p. 345.</span>"
        )
        assert citation == conference_citation(html=True, publication=record)

        record.authors = []
        citation = (
            '<span class="citation">Sagan C, Thorne K, editors. My title. <i>Proceedings of '
    ...
    def test_monograph_citation(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Battle J; My title. Series name. Hawking S, '
            "Wheeler J, editors. Baltimore: "
            "Doubleday; 2010 Feb. 5. Available at http://plone.org.</span>"
        )
        assert citation == monograph_citation(html=True, publication=record)
        ...
        record.authors = []
        citation = (
            '<span class="citation">Hawking S, Wheeler J, editors. My title. Series name. '
        ...
        record.title = ""
        citation = (
            '<span class="citation">Wohnlich E, Battle J; Series name. Hawking S, Wheeler J, '
    ...
    def test_report_citation(self):
        ...
        assert citation == report_citation(html=True, publication=record)
    ...
    def test_non_latin1(self):
        record = JournalRecord(
            title="My title",
            authors=[
                Person(last_name="Wohnliché", initial="E", first_name=""),
                Person(last_name="Carter", initial="G", first_name=""),
            ],
            journal="Sample Journal",
            pubdate="Jan 2007",
            volume="4",
        ...
        citation = (
            '<span class="citation">Wohnliché E, Carter G. My title. <i>Sample Journal</i> Jan 2007;4(5):345-7.</span>'
        )
        assert citation == journal_citation(html=True, publication=record)
    ...
    def test_html_chapter(self):
        record = ChapterRecord(
            ...
            authors=[
                Person(last_name="Zorin", first_name="", initial="AV"),
                Person(
                    last_name="Edler",
                    first_name="",
                    initial="L",
        ...
        citation = (
            '<span class="citation">Zorin AV, Edler L, Hanin LG, Yakovlev AY. Chapter 19. '
    ...
    def test_linked_journal_citation(self):
        record = JournalRecord(
            title="My title.",
            authors=[
                Person(last_name="Wohnlich", initial="E", first_name=""),
                Person(last_name="Carter", initial="G", first_name=""),
            ],
            journal="Sample Journal",
            pubdate="Jan 2007",
        ...
        assert citation == journal_citation(html=True, link=True, use_abstract=True, publication=record)
    ...
    def test_citation_from_journal_dataclass(self):
        ...
        assert (
            cit == '<span class="citation">Ministerial Meeting on Population of the Non-Aligned Movement '
            "(1993: Bali). Denpasar Declaration on Population and Development. <i>Integration</i> "
            "1994 Jun;(40):27-9.</span>"
        )
    ...
    def test_citation_from_chapter_dataclass(self):
        ...
        assert (
            cit == '<span class="citation">Kaefer CM, Milner JA, Benzie IFF, '
    ...
    def test_citation_from_book_dataclass(self):
        ...
        assert (
            cit == '<span class="citation">Ministerial Meeting on Population of the Non-Aligned Movement '
            "(1993: Bali). Denpasar Declaration on Population and Development. <i>Integration</i> 1994 "
            "Jun;(40):27-9.</span>"
        )
    ...
    def test_citation_html_title(self):
        ...
        citation = (
            '<span class="citation">Wohnlich E, Carter G. My &lt;title&gt;. <i>Sample Journal</i> '
